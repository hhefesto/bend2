...
function carb_book(src: Bend.Book, roots: Name[], js = false): Carb {
  book_owned(src);
  ...
  for (const queue = roots.slice(); queue.length > 0;) {
    ...
    term_any(cb, tld.h as HTerm, (s, tail) => {
      ...
      if (s.$ === "Ref") {
        ...
        if (FT_OPS.includes(op_name(s.k)) && intr_of(cb, s.k)) {
          cb.ftops.add(s.k);
        }
...
function emit_intr(fl: File, it: Intr, x: HTerm,
  ...
  // Each argument takes its parameter's layout; an array-valued op (the
  // fork's bulk ops) takes a record argument boxed (einsum's Es).
  const lays = sig_def(fl, k).lays;
  const ws = args.map((v, i) => Array.isArray(it.C) && v.ws.length > 1
    ? val_to(fl, v, BOX) : val_to(fl, v, lays[i]))
...
static bool corpus_grow(Corpus H, u64 need) {
  ...
  while (ok && need > a32_load(a32_at(H, H_CAP))) {
    ...
    char* at   = (char*)H + more;
    void* got  = io_gpu || ft_gpu || more >= 1ull << 43 ? MAP_FAILED
      : pool_try(at, more);
    ok = got == at;
    if (ok) {
      corpus_lay(H, more * 2);
